...
This code can be found [here](https://github.com/lukereding/utexas_r).
...
extractLargeMalePosition <- function(filename) {
  str_split(basename(filename), "_")[[1]][6]
}
...
getLatencyToEnterInteraction <- function(dataframe) {
  ...
  if(length(timeOfFirstInteractionZone) == 0) {
    timeOfFirstInteractionZone <- 600 + timeOfTrialStart
  }
...
alternate_df <- full_dataframe %>%
  select(filename, code, proportionTime, number_transits, scorer, half, experiment,fishID, largeMalePosition, prePost) %>%
  spread(code, proportionTime) %>% # create columns for each zone
