...
```{r message=FALSE}
 ...
 mutate(number_transits = map_dbl(file_contents, numberTransits),
         timeSpent = map(file_contents, getProportionOfTimeInEachRegion),
         latencyToEnterInteraction = map_dbl(file_contents, getLatencyToEnterInteraction)) %>%
  unnest(timeSpent) %>%
...
write_csv(full_dataframe, "full_may31_1303.csv")
...
alternate_df <- full_dataframe %>%
  select(filename, code, totalTime, number_transits, scorer, half, experiment,fishID, largeMalePosition, prePost, latency_plant, latency_right_screen, latency_left_screen) %>%
...
```{r}
write_csv(alternate_df, 'alternate_df_all_latency.csv')
```
